...
addEventListener('DOMContentLoaded', () => {
    ...
    // 🔥 COLE AQUI (ANTES DO animate)
    function mostrarNoiteEstrelada() {
    ...
    let portalAtivado = false;

    travelToggle.addEventListener("click", () => {
        if (portalAtivado) return;

        portalAtivado = true;

        travelToggle.classList.add("ativo");
        travelToggle.classList.remove("cursor-link-button");
        travelToggle.style.pointerEvents = "none";

        mostrarNoiteEstrelada();
        atualizarBotaoViagem();
    ...
    function atualizarBotaoViagem() {
        const icon = document.querySelector(".travel-icon");
        const isDark = document.body.classList.contains("dark-mode");

        if (isDark) {
            icon.src = "imgs/portal-noite.png";
        } else {
            icon.src = "imgs/portal-dia.png";
        }
    }

    function iniciarViagemDireta() {
        viagemConcluida = true;
        mostrarNoiteEstrelada();
        atualizarBotaoViagem();
    }
...
toggleButton.classList.remove("cursor-link-button");
...
toggleButton.classList.add("cursor-link-button");
